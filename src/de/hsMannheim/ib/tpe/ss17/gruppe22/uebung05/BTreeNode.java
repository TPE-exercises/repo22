...
/**
 * This class contains an implementation of the nodes of a B-Tree. The node
 * contains a number of values (degree - 2 * degree) and one more child than
 * values in the tree.
 *
 * Gruppe 2-3:
 *
...
public class BTreeNode {
    ...
    public BTreeNode(int degree) {
        ...
        values = new Comparable[(degree * 2) + 1];
        next = new BTreeNode[(degree * 2) + 2];
        this.degree = degree;
    }

    /**
     * This method returns whether the node is over filled (more than 2 * degree
     * values).
     *
     * @return true if over filled, false if number of values between degree and
     * 2 * degree.
     */
    ...
    @Override
    /**
     * Creates a string with a representation of the node in the format: [
     * elements ].
     */
    ...
    /**
     * Searches for the smallest element at the subtree (node and it's
     * children).
     *
    ...
    /**
     * Inserts an element at the right position in the node. The right and left
     * child of this value are set, too.
     *
     ...
     * @exception Throws an exception if the node is over filled or the value
     * already exists.
     */
    public void insert(Comparable o, BTreeNode rightPart, BTreeNode leftPart) {
        ...
        for (int i = values.length - 1; i >= 0; i--) {
            ...
            if (values[i] != null) {
                ...
                if (values[i].compareTo(o) >= 1) {
                    values[i + 1] = values[i];
                    next[i + 2] = next[i + 1];
                } else if (values[i].compareTo(o) == 0) {
     ...
     */
    public BTreeNode getNextNodeToSearch(Comparable o) {
        BTreeNode nextNode = next[values.length - 1];
        boolean found = false;

        for (int i = 0; i < values.length - 1; i++) {
            if (values[i] == null || o.compareTo(values[i]) <= -1) {
                if (!found) {
                    nextNode = next[i];
                    found = true;
                }
    ...
    /**
     * Splits the current node at the middle element in two new nodes. The
     * middle element gets inserted at the parent node and the references to the
     * two new subnodes are updated.
     *
    ...
    public void split(BTreeNode parent) {
        ...
        //Comparable middleElement = values[middlePosition];
